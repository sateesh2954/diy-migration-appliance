...
#This is the appliance script which will help to migrate IBM Cloud Classic VSI  and VMWare vm's to IBMCLOUD VPC
	...
	if [[ -f "$dirpath/migration.key" ]];then
		...
		if [[ "$sshres" == "y" ]];then
			...
			if [[ "$sshshowres" == "y" ]];then
				...
				printf "\n Please make sure, ssh public key content updated in $sshfilepath file"
			fi
		else
